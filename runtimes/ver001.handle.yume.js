...
export async function cli(fileUrl, block, argv) {
  ...
  switch (verb) {
    ...
    default:
      console.error(`yume: unknown verb '${verb}'. v001 supports: commit, history, heavy, heavy-apply, show, diff, rollback, trim, validate, refs, tags, impact, refs-check, note-add, note-edit, note-rm, note-list, notes-search, apply-list, apply-show, apply-index, apply-search`);
      process.exit(1);
  }
}
